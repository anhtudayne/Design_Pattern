...
| Bạn cần… | Package |
|----------|---------|
| DTO vào/ra | `com.cinema.booking.dto` |
| Chuỗi validate (CoR) | `com.cinema.booking.pattern.chain` |
| `PricingContext`, builder, strategy vé/F&B/time | `com.cinema.booking.pattern.strategy.pricing` |
| `IPricingEngine`, `CachingPricingEngineProxy` | `com.cinema.booking.pattern.proxy` |
| `PricingEngine`, decorator giảm giá | `com.cinema.booking.pattern.decorator` |
| Điều kiện cuối tuần/lễ (`PricingConditions`) | `com.cinema.booking.pattern.specification` |
| Entity JPA | `com.cinema.booking.entity` |
| Gọi full luồng tính giá | `com.cinema.booking.service.impl` |
...
- Mã giảm: **10% trên subtotal** (subtotal = \(165\)) → trong code: \(\lfloor 165 \times 10 / 100 \rfloor = 16\) (làm tròn xuống từng bước).
- Thành viên: **5% trên cùng subtotal** \(165\) (không phải trên tiền sau mã) → \(\lfloor 165 \times 5 / 100 \rfloor = 8\).
- **Tổng giảm** \(= 16 + 8 = 24\) → **còn phải trả** \(165 - 24 = 141\) (nếu không vướng trần; ở đây không vượt subtotal).
...
3. **PromoValidHandler:** Nếu có mã: mã có trong DB và chưa hết hạn? → `setPromotion`.

Nếu bất kỳ bước nào “cấm”, request **dừng** và báo lỗi — **chưa** tính tiền.

### Bước C — `BookingServiceImpl` sau validate

Trong **bản code hiện tại**, `promotion` nằm trong `PricingValidationContext` sau `PromoValidHandler` (chỉ kiểm tra tồn tại + hạn qua `PromotionRepository`). **Không** còn bước ghi đè qua service tồn mã riêng; nếu sau này khôi phục tồn kho, bước này có thể được thêm lại tại service.
...
2. **PromotionDiscountDecorator (nếu có promotion):** ví dụ 10% của **subtotal** (làm tròn xuống); cộng vào `promotionDiscount` / `totalDiscount`, không vượt **subtotal**.
3. **MemberDiscountDecorator (nếu có hạng):** % hạng cũng nhân với **cùng subtotal** (làm tròn xuống), cộng vào `membershipDiscount` / `totalDiscount`; tổng hai loại không vượt subtotal.

Engine gom kết quả trong `DiscountResult` (`promotionDiscount`, `membershipDiscount`, `totalDiscount`). `PriceBreakdownDTO` nhận `discountAmount` = tổng giảm và `membershipDiscount` riêng — **không** có field `promotionDiscount` trên DTO (phần mã nằm trong `discountAmount` trừ đi phần hạng).
...
| `Customer` | Tài khoản đang đăng nhập; chứa `membershipDiscountPercent` (% giảm theo hạng, null = chưa có hạng). | “An A đăng nhập, hạng Vàng giảm 5%.” |
...
| *(tồn mã)* | Trong repo hiện tại **chưa** có lớp service tồn mã trên luồng calculate; `PromoValidHandler` chỉ dùng `PromotionRepository`. | “Mã tồn tại và còn hạn.” |
...
| `PricingEngine` | Máy tính **thật**: cộng vé, F&B, phụ thu, trừ giảm. Dùng `EnumMap` để map strategy theo `PricingLineType`. | "Máy POS tính đủ công thức." |
...
| `MemberDiscountDecorator` | Trừ thêm theo hạng; % tính trên **subtotal** (chuỗi bọc: mã trong, hạng ngoài). | “Hạng Vàng: thêm một khoản % trên tổng trước giảm.” |
| `DiscountResult` | Bốn field: `totalDiscount`, `promotionDiscount`, `membershipDiscount`, `description`. | “Phiếu in: đã trừ từng loại.” |
...
**Nhớ:** `discountAmount` bao gồm cả giảm mã và giảm hạng; `membershipDiscount` chỉ là **phần hạng** trong đó. API **không** trả field `promotionDiscount`; nếu cần số tiền giảm mã riêng, lấy `discountAmount.subtract(membershipDiscount)` (tương ứng cách engine gán từ `DiscountResult`).
...
| `Promotion` | Một mã: **cách giảm** + hạn. | `code`, `discountType`, `discountValue`, `validTo` |
| `Customer` | User đăng nhập + **% giảm hạng**. | `userId`, `membershipDiscountPercent` |
...
**`Promotion`:** `code` — chuỗi mã; `discountType` + `discountValue` — giảm % hay số tiền; `validTo` — hết hạn. Trường tồn kho (nếu có trên entity) **chưa** được dùng trong `PromoValidHandler` hiện tại.

**`Customer`:** `userId` — phân biệt người (cache Redis khác nhau); `membershipDiscountPercent` — % giảm theo hạng, null = chưa có hạng (áp **sau** phần mã trong chuỗi decorator).
...
**Ai viết vào túi?** Handler suất ghi `showtime`; handler mã ghi `promotion`. **Ai đọc?** `PricingContextBuilder`.

| Field | Chỉ cần hiểu |
|-------|----------------|
| `request` | Phiếu `BookingCalculationDTO` gốc. |
| `showtime` | Suất đã load từ DB (sau bước 1). |
| `promotion` | Mã hợp lệ (sau bước 3, nếu có mã). |
...
> **Một câu:** Không nhập mã → **bỏ qua**. Có mã → mã phải **tồn tại trong DB và chưa hết hạn**.  
> **Không phải:** Không “nhẹ nhàng bỏ qua mã sai” — code **báo lỗi** nếu mã lỗi.

**Công cụ dùng:** `PromotionRepository` (`findByCode`, kiểm tra `validTo`).
...
### Tồn mã / kho khuyến mãi (trạng thái repo)

> **Hiện tại:** Luồng validate mã trong `PromoValidHandler` **không** gọi service tồn kho; chỉ kiểm tra mã có trong DB và chưa hết hạn. Mô hình “xem giá chỉ đếm / thanh toán mới trừ lượt” có thể được thêm lại ở tầng service sau này.
...
> **Một câu:** Nếu **lễ hoặc cuối tuần** → lấy **một %** (lễ được ưu tiên hơn cuối tuần; không cộng dồn) nhân với **tổng tiền vé** — **không** nhân với tiền bắp nước.

**Bean qualifier:** `@Component("tuTimeBasedPricingStrategy")`

| Thuộc tính | Chỉ cần hiểu |
|------------|----------------|
| `weekendSurchargePct` | % phụ thu cuối tuần — đọc từ `cinema.pricing.weekend-surcharge-pct` (mặc định 15). |
| `holidaySurchargePct` | % phụ thu ngày lễ — đọc từ `cinema.pricing.holiday-surcharge-pct` (mặc định 20). |

| Method | Chỉ cần hiểu |
|--------|----------------|
| `calculate(ctx)` | Không lễ/cuối tuần → 0; lễ → `holidaySurchargePct` × ticketTotal; cuối tuần → `weekendSurchargePct` × ticketTotal. |
...
**Câu chuyện:** Bắt đầu **chưa trừ gì**. Chuỗi decorator: lõi `NoDiscount`, bọc **mã**, bọc **hạng**. Cả **% mã** và **% hạng** đều lấy **subtotal** làm cơ sở (cộng dồn hai khoản, có trần subtotal) — không phải “% hạng trên tiền sau mã”.
...
> **Một câu:** Sau lớp trong, nếu có `Promotion` thì trừ thêm (PERCENT hoặc FIXED), không trừ quá subtotal.
> Ghi riêng vào `DiscountResult.promotionDiscount` để hiển thị tách biệt trên hoá đơn.

---

### `MemberDiscountDecorator` — vỏ **hạng**

> **Một câu:** Sau mã, nếu `Customer.membershipDiscountPercent` không null → trừ thêm % đó trên subtotal.
> Ghi riêng vào `DiscountResult.membershipDiscount`.
...
| Field | Chỉ cần hiểu |
|-------|----------------|
| `totalDiscount` | Tổng trừ = promotionDiscount + membershipDiscount. |
| `promotionDiscount` | Chỉ phần mã khuyến mãi. |
| `membershipDiscount` | Chỉ phần hạng thành viên. |
| `description` | Chuỗi mô tả áp dụng (đưa vào `PriceBreakdownDTO.appliedStrategy`). |
...
| Thuộc tính | Chỉ cần hiểu |
|------------|----------------|
| `strategiesByLine` | `EnumMap<PricingLineType, PricingStrategy>` — Spring inject `List<PricingStrategy>` qua constructor, tự động gom theo `lineType()`. |

**Khi app khởi động:** Constructor **kiểm tra** đủ 3 strategy (TICKET, FNB, TIME_BASED_SURCHARGE). Thiếu hoặc trùng → `IllegalStateException` (fail-fast, không đợi runtime lỗi).

| Method | Chỉ cần hiểu |
|--------|----------------|
| `calculateTotalPrice(ctx)` | Chạy 3 strategy → cộng subtotal → build Decorator chain → fill `PriceBreakdownDTO`. |
| `buildDiscountChain(ctx)` | NoDiscount → (có promotion?) PromotionDiscountDecorator → (có membershipDiscountPercent?) MemberDiscountDecorator. |
| `buildAppliedStrategyDescription(...)` | Sinh chuỗi mô tả đặt vào `appliedStrategy`. |

---

### `BookingServiceImpl` — hàm `calculatePrice`: **quầy chính**

> **Một câu:** Soát cửa → đóng hộp `PricingContext` (khi builder đã nối) → gọi máy tính (Proxy).

**Luồng đầy đủ (mục tiêu thiết kế):**

1. Tạo túi soát, chỉ có phiếu khách.  
2. Chạy 3 cửa soát — lỗi thì **dừng**; mã hợp lệ được gắn ngay trong `PromoValidHandler`.  
3. `pricingContextBuilder.build(validationCtx, request)` → `PricingContext`.  
4. `pricingEngine.calculateTotalPrice(context)` → `PriceBreakdownDTO`.

**Repo hiện tại:** có thể còn TODO (builder/engine chưa nối đủ); mở `BookingServiceImpl` để xem nhánh thực thi.

| Field (liên quan) | Chỉ cần hiểu |
|-------------------|----------------|
| `pricingEngine` | Thường là **Proxy** (có tủ nhớ). |
| `pricingValidationChain` | Đầu chuỗi soát. |
| `pricingContextBuilder` | Đóng hộp (inject khi dùng). |

| Method | Chỉ cần hiểu |
|--------|----------------|
| `calculatePrice` | Validate → (build context) → gọi `IPricingEngine`. |
...
BookingServiceImpl.calculatePrice
  → validate (chuỗi handler; promo gắn vào ctx nếu hợp lệ)
  → PricingContextBuilder.build (khi đã nối vào service)
...
6. **PricingContextBuilder.build(validationCtx, request):** đọc `ctx.showtime`, `ctx.promotion`, ghế, F&B, khách… từ DB → **PricingContext**. *(Nếu service chưa gọi builder, bước này bị bỏ qua — xem `BookingServiceImpl` hiện tại.)*
7. **CachingPricingEngineProxy.calculateTotalPrice(pricingContext):**
   - Ghép **khóa Redis** từ context.
   - Nếu Redis có DTO đúng kiểu → **trả luôn** (hết).
   - Nếu không → gọi **PricingEngine.calculateTotalPrice** (bước 8).
8. **PricingEngine:**
...
9. **Proxy** (nếu bước 7 là miss): **ghi** DTO vào Redis với TTL → trả về cho service.
10. **BookingServiceImpl** trả DTO cho client.

**Bạn có thể in đoạn 1–10** và gạch bút chì từng bước khi debug một request thật.
...
2. **`isEarlyBird` / `isHighOccupancy`:** Đã có trong `PricingConditions` nhưng **chưa** dùng để cộng phụ phí (reserved for future rules).
3. **`PromoValidHandler`:** Javadoc có thể khác **hành vi throw** trong code — tin code.
4. **UML:** Chỉ tên entity; chi tiết field ở **mục 1b**.
5. **`TimeBasedPricingStrategy`:** Phụ thu ngày lễ ưu tiên hơn cuối tuần — không cộng dồn cả hai. Nếu cần cộng dồn, mở rộng `calculate()` trong class này.
...
Đ: Engine xây chuỗi: lõi `NoDiscount` → bọc `PromotionDiscountDecorator` (nếu có promotion) → bọc `MemberDiscountDecorator` (nếu có hạng). **Thứ tự bọc** là mã trong, hạng ngoài; nhưng **công thức** trong code: cả hai đều dùng **subtotal** (vé + F&B + phụ thu) cho phần %, rồi cộng hai khoản (có trần không vượt subtotal) — không phải % hạng trên “tiền còn lại sau mã”.
