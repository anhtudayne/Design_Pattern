...
const STATUS_MAP = {
  PENDING:   { label: 'Chờ thanh toán', color: 'bg-yellow-100 text-yellow-700 border-yellow-200', icon: 'schedule' },
  PAID:      { label: 'Đã thanh toán',  color: 'bg-green-100 text-green-700 border-green-200',   icon: 'check_circle' },
  CANCELLED: { label: 'Đã hủy',         color: 'bg-red-100 text-red-700 border-red-200',         icon: 'cancel' },
};
...
export default function OrderLookup() {
  ...
  const handleSelectBooking = async (booking) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/tickets/booking/${booking.bookingId}`, {
        headers: getAuthHeaders(),
      });
      if (res.ok) {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-xs text-slate-400 font-medium ml-4">Tìm kiếm theo mã đơn hàng (Booking ID), số điện thoại hoặc email</p>
      </div>
      ...
      {!hasSearched && isDefaultList && results.length === 0 ? (
        <div className="text-center py-20">
          ...
          <div className="mt-10 grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto">
            {[
              { icon: 'confirmation_number', title: 'Mã đơn hàng', desc: 'Nhập ID booking (VD: 1, 2, 3...)' },
              { icon: 'phone', title: 'Số điện thoại', desc: 'Tìm theo SĐT khách hàng' },
              ...
              <div key={tip.title} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm">
                <span className="material-symbols-outlined text-2xl text-orange-500 mb-2 block">{tip.icon}</span>
                <p className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-tight mb-1">{tip.title}</p>
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-400 font-bold">Không tìm thấy kết quả cho "{query}"</p>
          <p className="text-xs text-slate-300 mt-2">Hãy kiểm tra lại thông tin và thử lại</p>
        </div>
      ) : (
              ...
              <div key={booking.bookingId} className="space-y-0">
                ...
                <button
                  ...
                  className={`w-full bg-white dark:bg-slate-900 rounded-2xl border shadow-sm hover:shadow-lg transition-all text-left p-6 ${
                    isSelected ? 'border-orange-500 shadow-orange-500/10' : 'border-slate-100 dark:border-slate-800'
                  }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center shrink-0">
                        <span className="material-symbols-outlined text-orange-500 text-xl">confirmation_number</span>
                      </div>
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-bold border ${status.color} flex items-center gap-1`}>
                ...
                {isSelected && (
                  <div className="bg-slate-50 dark:bg-slate-800/50 rounded-b-2xl border border-t-0 border-slate-100 dark:border-slate-800 p-6 -mt-3 pt-8 space-y-4">
                    {loadingTickets ? (
                      ...
                      <p className="text-center text-slate-400 text-sm py-4">Không có vé nào</p>
                    ) : (
                      <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {tickets.map(ticket => (
                            <div key={ticket.ticketId} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm">
                              <div className="flex items-center gap-2 mb-2">
                                <span className="material-symbols-outlined text-orange-500 text-base">local_activity</span>
                                <span className="text-xs font-black text-slate-800 dark:text-white uppercase">Vé #{ticket.ticketId}</span>
                              </div>
                              <div className="space-y-1 text-[11px] text-slate-500">
                                <p className="flex items-center gap-1.5">
                                  <span className="material-symbols-outlined text-xs text-slate-400">event_seat</span>
                                  Ghế: <span className="font-bold text-slate-700 dark:text-slate-300">{ticket.seatRow}{ticket.seatNumber}</span>
                                  <span className="text-[10px] text-slate-400">({ticket.seatType || 'Standard'})</span>
                                </p>
                                <p className="flex items-center gap-1.5">
                                  <span className="material-symbols-outlined text-xs text-slate-400">payments</span>
                                  Giá: <span className="font-bold text-orange-500">{formatMoney(ticket.price)}</span>
                                </p>
                              </div>
                            </div>
                          ))}
