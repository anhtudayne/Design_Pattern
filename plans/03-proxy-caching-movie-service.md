...
|----------------|---------|
| `MovieService` | **Interface** (san co `com.cinema.booking.services`) |
| `MovieServiceImpl` | **Concrete** — delegate that; bean name ro (`@Service("movieServiceImpl")` hoac tuong duong) |
| `CachingMovieServiceProxy` | **Proxy** — `implements MovieService`, cache Redis, invalidate sau CUD |

**Duong dan proxy:** `backend/src/main/java/com/cinema/booking/patterns/proxy/CachingMovieServiceProxy.java`

**Mapping domain:** [Movie](../classdiagram.md) trong `classdiagram.md`; proxy bao quanh dich vu doc phim, khong doi entity.

---

## Clean Code va SOLID

- **S:** Proxy chi cache + delegate; khong copy paste body `MovieServiceImpl`.
- **O:** Mo rong policy cache (TTL, key) trong proxy hoac properties.
- **L:** Proxy thay the impl hop le voi moi caller `MovieService`.
- **D:** Proxy va controller phu thuoc `MovieService`; dung `@Qualifier` tranh vong bean.

**Clean Code:** Hang so / property cho key prefix va TTL; invalidate tap trung.
...
- [x] Proxy implement đầy đủ method `MovieService`
- [x] Controller dùng proxy transparently (nhờ `@Primary`)
- [x] Invalidate sau CUD
- [x] Build/test pass
